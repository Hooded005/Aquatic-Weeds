...
def prediction_view(request):
    ...
    if request.GET.get('keep_data') == 'true':
        data = [
            {'Date': pred[0],
             'Temperature': pred[1],
             'Wind_Speed': pred[2],
    ...
    my_context.update(redo_map(array_pos))
...
def redo_map(pos):
    ...
    context = {'coverage_percentage': str(coverage_percentage) + '%',
               'coverage_description': coverage_description(coverage_percentage),
               'image_path': coverage_image(coverage_percentage), 'coverage_date': my_predictions[pos][0]}
    print(context)
    return context
